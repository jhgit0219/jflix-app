...
export class KidsComponent implements OnInit {
  ...
  async loadGenres() {
    try {
      ...
      // Fallback to default genres for kids content
      this.genres = [
  ...
  async loadMovies(reset = false) {
    ...
    try {
      ...
      // For kids content, we always filter by family-friendly genres
      if (this.selectedGenre) {
        endpoint = `${baseUrl}/api/movies/genre/${this.selectedGenre}?page=${this.page}&certification=G,PG`;
      } else {
        // Default to family/animation genres for kids
      ...
      this.error = 'Failed to load kids content. Please try again later.';
    } finally {
      this.loading = false;
    }
  }
